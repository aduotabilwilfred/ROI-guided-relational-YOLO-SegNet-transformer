...
PAPER_TRAIN_ARGS = {
    ...
    "optimizer": "AdamW",
    "lr0": 1e-4,
    "lrf": 0.01,  # final LR factor; with cos_lr gives the cosine decay
    "cos_lr": True,
    "imgsz": 512,
    "degrees": 10.0,
    "fliplr": 0.5,
...
def evaluate_pooled(
    ...
    for fold in range(n_folds):
        ...
        for record in test_records:
            ...
            image_file = next((test_dir / "images").glob(f"{stem}.*"))
            label_file = test_dir / "labels" / f"{stem}.txt"

            result = model.predict(
                str(image_file), imgsz=size, device=device, verbose=False
            )[0]
